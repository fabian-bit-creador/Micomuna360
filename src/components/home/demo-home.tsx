...
export async function DemoHome({ commune }: { commune: CommuneConfig }) {
  const base = `/${commune.id}`;
  const [news, events, indicators] = await Promise.all([
    getLatestNews(commune.id, 3),
    getUpcomingEvents(commune.id, 2),
    ...
    <>
          ...
          <div className="max-w-2xl">
            ...
            <div className="mt-8 flex flex-wrap gap-3">
              <Button size="lg" asChild>
                <Link href={`${base}/reportar`}>
                  <MessageSquarePlusIcon />
                  Reportar un problema
                </Link>
              </Button>
              <Button size="lg" variant="outline" asChild>
                <Link href={`${base}/telefonos`}>
                  <PhoneCallIcon />
                  Teléfonos útiles
                </Link>
              </Button>
      ...
      <section className="mx-auto max-w-6xl px-4 py-14">
        ...
        <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
          {accesos.map((acceso) => (
            <Link key={acceso.href} href={`${base}${acceso.href}`} className="group">
      ...
      <section className="border-b bg-background">
        <div className="mx-auto max-w-6xl px-4 py-12">
          <SectionHeader
            ...
            action={{ label: "Ver todos los datos", href: `${base}/datos` }}
          />
      ...
      <section className="mx-auto max-w-6xl px-4 py-14">
        <SectionHeader
          ...
          action={{ label: "Ver todas", href: `${base}/noticias` }}
        />
        <div className="grid gap-4 md:grid-cols-3">
          {news.map((article) => (
            <NewsCard key={article.id} article={article} communeSlug={commune.id} />
          ))}
        </div>
      </section>
      ...
      <section className="mx-auto max-w-6xl px-4 pb-14">
        <SectionHeader
          ...
          action={{ label: "Ver agenda completa", href: `${base}/actividades` }}
        />
